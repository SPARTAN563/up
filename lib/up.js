...
function UpServer (server, file, opts) {
  ...
  opts.allocator = opts.allocator || 'socket.io';
  if(opts.allocator == 'socket.io')
    this.allocator = /^\/socket\.io\/[^\/]*\/(?:xhr-polling|htmlfile|jsonp-polling)\/([^\?]+).*/;
  else if(opts.allocator == 'sockjs')
    this.allocator = /^(?:\/[^\/]+)*\/(\d+)\/[a-z0-9_]+\/(?:websocket|xhr_streaming|xdr_streaming|iframe_eventsource|iframe_htmlfile|xhr_polling|xdr_polling|iframe_xhr_polling|jsonp_polling).*/;  
  else if(opts.allocator instanceof RegExp)
    this.allocator = opts.allocator;
  else
    this.allocator = new RegExp(opts.allocator.replace('/', '\\/'));
...
UpServer.prototype.defaultWS = function (req, res, next) {
  var matcher = req.url.match(this.allocator);
  if(this.workers.length && matcher && matcher.length > 1) {
    // transport = matcher[1], sid = matcher[2]
    var sid = matcher[1];
    if(this.debug) console.log(sid);
